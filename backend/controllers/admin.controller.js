...
export const uploadQuestionPaper = async (req, res) => {
  try {
    ...
    if (admin.isadmin) {
      ...
      const uploadQuestionPaper = await Result.create({
        author: userId,
...
export const editQuestionPaper = async (req, res) => {
  try {
    ...
    console.log(year);
    const admin = await User.findById(userId);
    const questionpaper = await Result.findById(questionpaperId);
    if (admin.isadmin) {
      if (year) result.year = year;
      if (branch) result.branch = branch;
      if (sem) result.sem = sem;
...
export const deleteQuestionPaper = async (req, res) => {
  try {
    ...
    const questionpaperId = req.params.id;
    const admin = await User.findById(userId);
    const questionpaper = await Result.findByIdAndDelete(questionpaperId);
    if (admin.isadmin) {
...
export const uploadEvents = async (req, res) => {
  try {
    const userId = req.id;
    const { formlink, description, lastdate } = req.body; // Now accepting lastdate
    const event = req.file;
    const admin = await User.findById(userId);

    if (admin.isadmin) {
      const dataUri = datauri(event); // Changed from `result` to `event`
      let cloudResponce = await cloudinary.uploader.upload(dataUri, {
        resource_type: "auto",
      });
